...
- `main`은 항상 검증 green 상태를 유지한다.
- 코드 구현은 WBS 항목 단위의 짧은 브랜치에서 진행한다: 기능 `feat/<unit>-<항목>`,
  수정 `fix/<주제>`, 문서 `docs/<주제>`.
- 구현 브랜치는 에이전트 리뷰를 통과한 뒤 `main`에 병합한다(현재 단일 작업자라
  squash 또는 fast-forward 허용). 문서·소규모 변경은 `main` 직접 커밋을 허용한다.
