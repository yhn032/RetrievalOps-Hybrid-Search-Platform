...
## Status: in-progress — 표준 00~09 승인·WBS 간트(Excel) 전환 완료. 잔여: app 스캐폴드 구현(GR, M1 의존)
...
| GR7 | 결정 반영 스캐폴드(WBS 0.4) | 2026-07-02 | web-ui·model-serving·message-queue 신설, index-worker Java 전환, retrieval-service ADR-0007 정합, runtime 검사 27파일+인프라 USER 보강(G-P8-3). 검증 126 PASS·runtime 27·링크 59 |
| GR8 | GR7 evaluator 교차검증 + 문서 정합 | 2026-07-02 | 6-렌즈 적대 검증 통과(게이트 전부 green, 발명·비밀 0). G-P8-1·G-P8-3 해소 확정. 지적된 PROJECT.md 산문 6→9단위, ADR-0007 증분 명기 보정 |
...
| GR6 | WBS 간트차트 Excel 전환 (R1) | 2026-06-30 | wbs.xlsx 신설(WBS·간트차트 2시트, 32작업, 마일스톤 색상), wbs.md는 텍스트 분해 보존+xlsx 링크. 일정은 계획 추정(D+0=2026-07-01) |
...
| GR | 잔여 gap 해소 + app 스캐폴드 구현 | P8 | — | 스캐폴드 완료(GR7), evaluator 교차검증 통과·G-P8-1·G-P8-3 해소 확정(GR8). ★ 재개 지점: M0 잔여(WBS 0.2 설계·0.3 데이터셋/질의셋 ADR) → M1 의존 구현(1.1 등, 사용자 구현 영역) |
...
| # | gap | 심각도 | 관련 | 상태 |
|---|---|---|---|---|
| G-P8-1 | 모델 실행 별도 컨테이너 부재 — anchor와 WIP 계획은 "모델도 별도 컨테이너 기동"을 요구하나 임베딩·Reranker가 retrieval-service에 in-process로 접힘. ADR·gap 기록 없음 | 높음 | P3·P5 | 해소(GR7: app/model-serving 단위 신설, retrieval-service·index-worker가 MODEL_SERVING_URL REST 호출, ADR-0007. evaluator 교차검증 확정) |
...
| G-P8-3 | metadata-store(MariaDB) 3개 Dockerfile에 명시 USER 없음(entrypoint가 root로 시작 후 mysql로 drop). runtime-rules-check가 인프라 USER를 미검사하는 사각 포함 | 중간 | P6·P7 | 해소(GR7: runtime-rules-check가 인프라 단위에 USER 또는 권한 드롭 주석을 27파일 전수 검사, 역-테스트로 강제 확인. evaluator 교차검증 확정) |
