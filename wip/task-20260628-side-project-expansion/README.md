...
| # | gap | 심각도 | 관련 | 상태 |
|---|---|---|---|---|
| G-P8-1 | 모델 실행 별도 컨테이너 부재 — anchor와 WIP 계획은 "모델도 별도 컨테이너 기동"을 요구하나 임베딩·Reranker가 retrieval-service에 in-process로 접힘. ADR·gap 기록 없음 | 높음 | P3·P5 | open(결정 필요) |
...
| G-P8-3 | metadata-store(MariaDB) 3개 Dockerfile에 명시 USER 없음(entrypoint가 root로 시작 후 mysql로 drop). runtime-rules-check가 인프라 USER를 미검사하는 사각 포함 | 중간 | P6·P7 | open |
